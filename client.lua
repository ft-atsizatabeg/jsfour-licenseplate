...
AddEventHandler('jsfour-licenseplate', function()
  ...
  ESX.UI.Menu.Open(
    ...
    align    = 'bottom-right',
		elements = {
			{label = 'Plakayı değiştir', value = 'change'},
			{label = 'Onayla', value = 'apply'},
		}
	},
	function(data, menu)
      ...
      if plate ~= nil then
        ...
        if distance ~= -1 and distance <= 3.0 then
          ...
          ESX.TriggerServerCallback('jsfour-licenseplate:update', function( cb )
            ...
            elseif cb == 'error' then
			  ...
			  exports['mythic_notify']:DoHudText('error', 'Plakayı değiştiremediniz!')
            end
          end, oldplate, plate)
        else
		...
		exports['mythic_notify']:DoHudText('error', 'Boş plaka değiştiremezsiniz!')
      end
		end
	end,
	function(data, menu)
